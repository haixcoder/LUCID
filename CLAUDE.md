...
- Workflow 全功能对齐调研：`docs/workflow-alignment-research.md`（**v2.1，2026-09-09 收口版**，证据基座 **Claude Code 2.1.263**、实施基线 **Lucid 1.2.64**：CC Workflow 能力分四层——L1 脚本语言面（`meta{name,description,title,whenToUse,phases[{title,detail,model?}]}` **7 字段**、`agent(prompt,{label,phase,schema,model,effort,isolation,agentType})` 7 选项 + 3 未文档化、`pipeline`(无栅栏,回调收 `(prev,orig,index)`)、`parallel`(栅栏,null 不 reject)、`phase`/`log`/`args`/**`budget{total,spent(),remaining()}` 硬上限**/**`workflow()` 子流仅一层**、沙箱禁 `Date.now/Math.random/new Date` 与 `import()`）、L2 运行面（journal 的 `v2:<sha256>` 即 resume 缓存键、实测并发上限 16、`meta.phases` 单独就能出阶段进度）、L3 调用面（工具入参 `script|name|scriptPath|args|resumeFromRunId`）、L4 分发面（`.claude/workflows` / `~/.claude/workflows` / 插件 `workflows` 清单字段 + `/plugin:name` 命名空间）；**证据源=官方页 curl + `strings` 还原内置 `workflow-authoring` 技能全文（脚本 API 唯一权威，官方页列不全）+ 本机 5 次真实运行 + 官方插件 7 份脚本**；**对齐设计=三层表达 + 一条诚实出口**（A 结构层可视化 / B 组合层半可视化 `map`=pipeline、`branch`=if、`loop`=while、`retry`/`log`/`subflow` / C 代码逃生舱 `code` 节点 / S 仅呈现：命令区·成本条·文案），**语义覆盖 100% 靠 A∪B∪C；"可视化覆盖率"必须带分母——结构构造 10/10、agent 选项 7/7（map 级 1 同享）、meta 7/7 全部可视化，残留（任意表达式/深层嵌套/budget 动态规模/未文档化 opts）刻意归 C 层**（v1.0 的"~85%"无分母，v2.0 已废弃）；实施记录 X6–X12 ↔ 版本 ↔ commit 表、对齐矩阵 **37 行**、单点清单、**886 断言**测试矩阵、**§9.D 二进制级复核勘误表（v2.0→v2.1 九条）**；**覆盖核对清单在 §2.5，逐条"到没到 100%"看它**）
...
2. **只读数据源**：扫描 `~/.claude/projects/` 永远只读。服务写域（1.2.59 起）= `~/.claude/cc-viewer/`（config.json / sent.json / server.pid / cas.pem / drafts/）**＋ 保存草稿时那个 cwd 下的 `.claude/workflows/<name>.js`**——用户 2026-09-09 明确要求"保存后放入项目的 workflow 下"，写域由 `web._project_workflow_path` 单点收口（绝对路径 + 已存在目录 + `.claude`/`.claude/workflows`/目标文件**三级软链拒绝** + name 白名单 + 只写执行件、不写图草稿；同名**直接覆盖**，与草稿目录的"宁并存不覆盖"相反）。除此之外任何路径都不许写。
...
5. **回归测试已入库，`python3 tests/run_all.py` 是准入门槛（1.2.18 起）**：
   ...
   - `tests/frontend/test_*.ts`（前端一律 TypeScript，1.2.31 起；开发期依赖 node——运行时仍零依赖）：node 原生 type-stripping 直跑 .ts（**需 node ≥22.18**，无编译步骤；版本过旧 run_all 警示并跳过）；`harness.ts` 无头 DOM 桩加载**真实编译产物**驱动 render/diff/抽屉/轮询；**桩的 `fire()` 同时调用 `addEventListener` 监听者与 `on<type>` 属性处理器**（浏览器语义——只认前者会让"用 `.oninput =` 绑的输入框"在桩里永远点不动，1.2.64 实际踩到）；`test_render_golden.ts` 为 card/sessCard 出 HTML 的黄金快照——**任何前端重构前后黄金必须逐字节一致**（故意改文案/结构才 `UPDATE=1` 重录并在提交注明）；类型检查由 `typecheck.ts` 套件代跑（`tsc -p tsconfig.check.json`，需开发机 `npm install` 一次；无 node_modules 则警示跳过）；
...
| `web.py` | HTTP handler `class H`、`make_server()`，启动时读入 `static/index.html`（**改了 static 下产物必须重启服务才生效——INDEX_HTML 是 import 期读的**）。编排器三单点全在本模块（handler 只调用，不写第二份口径）：`static_path`（白名单静态件：末段取名 + 枚举 + resolve 后 is_relative_to 软链双保险）、`_draft_dir`（cwd→`<sanitize>-<hash6>`，**cwd 永不作为写路径成分**；空 cwd 落 `__no_cwd__` 常量而非进程工作目录——`realpath('')` 会回落 cwd，保存与回载就可能不同目录=存了却看不见）、`save_draft`（`.js`+`.json` 成对 `tmp→os.replace` 原子落盘；同名异内容默认并存 `<name>-<sha8>.*`，`overwrite:true` 才覆盖；name 白名单 `[A-Za-z0-9][A-Za-z0-9._ -]{0,79}\Z`；脚本空/超 1MB/`draft_ver_ok` 不过（`DRAFT_VERS=(1,2)`，1.2.50 起）/非 dict 全拒；OSError 回 `ok:false` 不打 500）、`list_drafts`（**目录枚举本身即口径**，坏件静默跳过，`DRAFT_LIMIT=200`）、`_project_workflow_path`（项目分发落点单点，1.2.59）、`list_agent_types`（`GET /api/agents`，只读枚举 agentType 候选，见下）、`list_workflows`（**工作流库**列表，1.2.71：`CONF_DIR/drafts/*/*.json`（跨项目草稿，cwd 存在 JSON 里）∪ `<cwd>/.claude/workflows/*.js` ∪ `~/.claude/workflows/*.js` ∪ `PROJ/*/*/workflows/wf_*.json`（按 workflowName 去重留最近）四类只读枚举；`graph` 徽标 = 内嵌注释探测，`EMBED_RE` 单点）、`read_workflow`（库单件读取：`path` 必须落在白名单根内——`CONF_DIR/drafts/**`、`PROJ/**/workflows/{wf_*.json,scripts/*.js}`、任意 `<dir>/.claude/workflows/*.js`——越界一律 None→404；读取上限 `LIB_READ_CAP=1MB` 且截断如实上报） |
...
- **`FULL` 缓存三态必须全部参与渲染**（1.2.61，真实反馈「点击展开详情后，不显示内容」）：`{有全文 / m:true(数据源超限) / 未拉取}`——渲染点只读"有全文"那一支、其余两态靠 `onToggle` 临时写 DOM 的，下一轮轮询重建必被冲掉（症状=静默空白，不是报错）。四个消费者 `turnHead`/`waitline`/`stepRow`/`card` 的 agent 行**逐一对齐**（1.2.61 前 `stepRow` 缺 miss 分支：`fu.r=''` 让 `R` 恒空 → OUT 面板整块消失）；「该步」文案单点 `MISS_STEP_TXT`。测 `tests/frontend/test_fulltext_drawer.ts` 的步骤行 miss 三断言（含"轮询重建后仍在"）。
...
- **map 链分析单点 `flowMapChain`**（1.2.53）：从 map 出发沿"单入口后继"贪心推进，**级 1 = map 节点自身的 prompt/label 模板**（PRD §5.1 草稿形状），下游每级 = 单个 agent 或"扇出 → 同一 merge"；终止于 return / 多入边汇合 / 扇出级 merge（其后是消费者）。生成时链内节点不再出顶层 `const`（`flowMapChains` 汇总 inner 集），只由 `pipeExpr` 在 pipeline 回调里消化；回调签名统一 `(prev, item, i)`，`{{item}}`→`item`、`{{index}}`→`i`、`{{上一级任一节点}}`→`prev`，**链内引用非上一级 = 校验报错**（否则生成物 ReferenceError）。`map` 独占一个 await 点（不许与其他步骤同层）。
- **分支区域单点 `flowBranchRegion`**（1.2.54）：true/false 各自走 `flowWalkStages`（**与 map 链共用同一走链器**，`firstIsStage=true`），要求两条都连、可达集不相交、终止于**同一个** merge（或同一个 return）。生成 `let <merge>; if (cond) { … } else { … }`——区域末级结果**直接赋给 merge 变量**（不留中间 `const`），块内引用同区域更早节点用真实变量名（不是 pipeline 的 `prev`）。`branch` 同样独占一个 await 点；区域内变量出块即失效 → 链外引用由 `flowRegionOf` 单点拦截。`branch` 出把 `true`/`false`、`merge` 入把 `in`/`in2`（data-id 拼法不变，见 `handleHTML`）。
- **循环区域单点 `flowLoopRegion`**（1.2.55）：回边 = **指向 loop 节点、且来源在它体内**的边（不需要专用 handle，用户把体末连回 loop 即得）；`flowBackEdgeIds` 让 `flowLevels` 忽略回边——**合法循环不再被当成环报错，而"不在任何 loop 体内的环"照旧报错**。区域规则 1：体首入边只许来自 loop（单入口）、体末必须且只能回到 loop（单出口）。生成 `let <loop>; { let round = 0; while ((cond) && round < maxRounds) { … } }`（块作用域隔离多个循环的 round），勾预算守卫才出 `if (budget.total && budget.remaining() < 50000) { log(…); break }`。
- **代理数估算单点 `flowAgentEstimate`**（1.2.55）：每个 agent 计 1 × 它所在**全部** loop 的 maxRounds 之积；成本条（`#fCost`，`renderCost`）据此显示 `◆N · 并发≤16 · 循环上界≤M · 预算守卫`，`≥25` 加 `.over` 变红（对齐官方 `Large workflow` 告警），title 写明"估算值 + 上限"。
- **agent 选项 / retry / log（1.2.56）**：`opt()` 单点拼 `label/phase/model/effort/agentType/isolation/schema`；**`phase` 恒发**（单节点层也发——1.2.56 口径变更，之前只有并行层发；`phase()` 全局状态在 pipeline/parallel 内有竞态）。`retryN>0` 的节点走 `$retry(prompt, opts, n, backoffMs)`，助手**全图只生成一次**（形状同官方 `scan.js`：失败 → `log` 旁白 → 指数退避 `setTimeout`）；`retryN/retryMs` 必须 ≥0 整数。`log` 节点出 `log(\`…\`)`（`{{nX}}` 走同一 `flowLiteral`）。`agentType` 候选来自后端 `GET /api/agents`（`web.list_agent_types`：只读枚举 `~/.claude/agents/*.md` + 插件 `cache/<market>/<plugin>/<ver>/agents/*.md` 的 `<plugin>:<name>`，`AGENT_TYPE_CAP=200`）——**下拉只是起点，自由文本永远可用**。
- **code / subflow 校验（1.2.57）**：`flowStripLiterals` 把字符串/注释换成等长空白（正则字面量**不识别**——那正是"无法判定"的来源）；`flowCodeErrs` 只对去字面量后的片段判 `import(` 与 `Date.now()/Math.random()/new Date()`（命中在字面量里 → 降级为**警告**）；`flowWarnings(fs)` 是与 `flowValidate` 并列的**第二类结果**（错误拦生成、警告只提示），未知全局（不在白名单/内建/本片段声明/节点 id 内）走它。`flowSubflowErrs(fs, ctx)` 查 ref 必填 + 按名引用时的存在性与**一层嵌套**（`FlowCtx.drafts` 由 `flowCtx()` 单点从 `fDrafts` 生成，`nested` = 该草稿内部还有 subflow）；路径引用不做存在性检查。产物沙箱禁令改成**去字面量后**判定（code 片段里的 `'Date.now()'` 是数据不是调用）。
- **生成器三口径各一个函数**：`flowValidate`（错误清单，空=可生成）/ `flowMetaPhases`（顶部 PHASES 条与脚本 `meta.phases` 共用它；1.2.50 起**显式阶段带优先、空/全空标题回落推导**，显示清单走 `flowBandPhases`＝显式原样（含正在编辑的空标题行）或推导，出码清单才过滤空标题）/ `flowGenerate`（出码，只有内部 bug 才抛）。UI 与测试只调这三个，禁止另数 phase 或自己判分层；产物不许含 `Date.now()/Math.random()`（沙箱禁用且破 resume）。**`meta` 的空字段不落码**（`whenToUse`/`detail`/`model` 为空即不出该键）——这是"v1 草稿产物逐字节不变"的唯一实现方式，`tests/frontend/golden/flowgen_demo.js` 钉死。
- **`meta.phases[].model`（1.2.63，技能原文「Add `model` to a phase entry when that phase uses a specific model override」）**：它是**阶段元数据**（二进制复核：`L()` 解析保留但**全库无消费者**），**不改变任何 `agent()` 调用**——生成器只把它写进 `meta.phases`，编辑器不据此改节点 opts。四处一起改的形状：类型 `FlowPhase{title,detail?,model?}` / 落码 `flowMetaPhases`（键序 title→detail→model，**detail 空而 model 非空时不许吞掉 model**）/ **存储形态单点 `flowPhaseRow`（空 model 不落盘——打开旧草稿再保存不该平白多出 `"model":""`）** / 合法性判定单点 `flowModelBad`（agent.model 与 phases[].model 共用，空值一律放行）。UI 是阶段带第三列 `input.ph-m` + `datalist#fModels`（自由填 `claude-*` 全 id 也可）。测：`test_flowgen_meta.ts` 6 断言 + `test_flow_editor.ts` 三列编辑/存储/回载 + `?flowsmoke` 的 `meta-v2`。
- **`meta.title`（1.2.64）**：二进制归一化函数 `E()` 实证 meta 读 **5 键** `{name,description,title,whenToUse,phases}`（v1.0/v2.0 清单都漏了它）。头部第四输入 `#fTitle`，与 `whenToUse` 同规矩"空不落码"；**只做输入与落码，不猜展示用途**（无消费者）。测 `test_flowgen_meta`（落码/空不落/沙箱）+ `test_flow_editor` + `?flowsmoke` 的 `meta-v2`。
- **`flowIsAgentNode` 单点（1.2.64）**：会产出 `agent()` 调用的节点型 = `agent` **+ map 节点自身**（map 链的级 1 就是它，走同一个 `opt()`/`call()`）。**SCHEMA_ 常量生成、schema/model/effort/retry 校验、检查器字段三处共用这一个判定**——旧实现只认 `type==='agent'`，导致"map 带 schema"会生成引用未定义 `SCHEMA_<id>` 的脚本。配套：**检查器字段单点 `optFieldsHTML`**（agent 与 map 共用同一份 HTML，含把自定义 `claude-*` 全 id 补进 `<select>` 的选项——否则回载显示 inherit、一碰就覆盖丢值）。测 `test_flowgen_pipeline` + `test_flow_editor`。
- **`flowPhaseGroups` 单点（1.2.64）**：生成物里**实际会出现的进度组名** = 每个含 agent 的层的 `phase||label` + 所有 agent/map 的显式 `opts.phase`（1.2.56 起 phase 恒发）。阶段带推导（`flowDerivedPhases`）与**失配警告 `flowPhaseBandWarns`** 共用它——显式阶段带标题与分组不同名时，运行期会多出一个空进度组（官方：声明了的 title 照样出组、不匹配的 `phase()` 自成一组），双向各给一条**警告**（不拦生成）。测 `test_flowgen_meta` 4 断言。
- **成本条口径（1.2.55；1.2.64 修分支重复计数）**：`flowAgentEstimate` = agent 节点数 × 所在全部 loop 的 `maxRounds` 之积，**分支区域按臂取 max**（if/else 只跑一条，求和会系统高估）；**不计** map 的条目基数、子流内部代理、retry 尝试——这三条在成本条 `title` 里明写。改口径要连 title 文案与 `test_flowgen_allnodes`/`test_flowgen_branch` 一起改。
- **字段选择（1.2.60，真实需求「一个节点有多个字段输出，下一个节点如何接入」）**：占位符支持**字段路径** `{{nX.字段}}` / `{{nX.list[0]}}` / `{{item.字段}}` / `{{prev 级.字段}}`，生成 `${nX?.字段}`（全程可选链——上游被跳过时 agent 返回 null，取字段不该炸整张图）。语法单点 = `FLOW_REF_PATH`（**必须带捕获括号**，写成 `(?:…)` 会让 `m[2]` 恒 undefined、三个字段引用被当成同一个整节点引用——实现期实际踩到）+ `flowPathExpr`（`.a`→`?.a`、`[0]`→`?.[0]`，路径段白名单 = 标识符/数字下标，即注入防线）；整节点引用（无路径）产物**逐字节不变**（黄金钉死）。诊断单点 = `flowRefIssues`（**错误与警告同一次扫描**，`flowValidate` 取 errs、`flowWarnings` 取 warns）：① `{{nX.1bad}}` 形似引用但不合规 → 错误（旧实现原样当字面量喂给子代理，静默坏）；② `{{nX}}` 指向声明了 schema 的节点 → 错误（模板字符串把对象变 `[object Object]`，静默坏）；③ `start`/`index` 取字段 → 错误；④ 取字段但上游无 schema / 字段名不在 `properties` → 警告（`flowSchemaProps` 单点，解析不了就不猜）。编辑器「字段选择」芯片 = `flowFieldChips`（候选：上游节点 ∪ map 链内 `item`/`index`；点一下插到 prompt 光标处，`refreshFieldChips` 只重出芯片容器不整重渲染）。**新增取字段写法时，四处一起改**：正则/`flowPathExpr`/`flowRefIssues`/芯片。测：`tests/frontend/test_flowgen_fields.ts`（13 断言：生成/沙箱/校验/警告）+ `test_flow_editor.ts` 芯片六断言 + 真机冒烟 `✓field-chips`。
- **工作流库三态 + 内嵌图(1.2.71)**：顶栏「工作流库…」开 `#fLibPanel`（浮在画布上，绝不进 diffPaint 池）。打开一条 = **单点顺序**的三态：① 有图（草稿 JSON / 执行件尾部 `// lucid-graph:1:<base64>` 注释——编解码住 `flowEmbed`/`flowDecodeGraph`）→ 无损上画布；② 只有脚本 → `flowParseVerified`（住 45-flowgen）= `flowParse` 按**生成器文法**反解 + `flowGenerate(还原图) === 原文` **逐字节复核**，不过一律 null（宁可不还原，不给与脚本对不上的假图）；反解器只认生成器锚点（首行注释），`flowParseDbg` 留最近失败原因供诊断；③ 反解不了 → 只读全文 + 写明原因（铁律 7）。**载入即切上下文**（用户 2026-09-19 拍板）：图自带 cwd 时 `FS.cwd` 随之切换，头部/草稿库/保存落点/命令区全跟随、保存落回原项目；没有 cwd 不切、不把上下文清空。`flowEmbed` **只加在保存载荷**（`saveFlowDraft` 里 `gen + flowEmbed(d)`；>950KB 宁可不嵌并在状态行写明）——`flowGenerate` 产物逐字节不变（黄金钉死，预览也不含 base64）。测：`tests/frontend/test_flowparse.ts`（往返/拒绝面/内嵌，34 断言）+ `test_flow_editor.ts` 库三态 + 后端 `tests/test_workflow_lib.py`；真机冒烟 `✓embed` `✓lib-open`。
- **草稿版本判定两端同源**：后端 `web.draft_ver_ok`（只认整数白名单 `DRAFT_VERS=(1,2)`，`bool` 是 `int` 子类须显式排除）与前端 `loadFlowDrafts` 守卫（`v!==1 && v!==2`）是同一口径的镜像，改一端必须同时改另一端 + `test_draft_api.py` / `test_web_api.py` / `test_flow_editor.ts` 三处用例（1.2.50 v2 = 纯加宽：`whenToUse` + `phases[{title,detail}]` + `argsSpec`，v1 原样载入，未知版本仍拒绝不猜）。
- **args 契约两单点**（1.2.51）：`flowArgsSpec`（读取与缺省，生成器/校验器/UI 共用）+ `flowSchemaErrs(where, text)`（**JSON Schema 根形状校验**，`agent.schemaText` 与 `argsSpec.schemaText` 共用同一实现，只换报错前缀——运行期只接受 `{type:'object',properties:{…}}` 且 `required ⊆ properties`）。生成物形状：`const ARGS = (typeof args === 'string') ? JSON.parse(args) : args`，勾必填才出前置校验块（错误信息含**缺失字段名**）；有 ARGS 时 `Q` 从 `ARGS` 派生，保证"对象 args"与"字符串化 JSON args"两条通路落到同一入口（沙箱断言钉死）。`argsSpec` 的 schema/example 存 **JSON 原文**（`schemaText`/`exampleText`，与 `agent.schemaText` 同一约定：编辑器是文本框，存文本才不丢半截 JSON；落盘前 `flowValidate` 已保证合法）。
- **桩替身沙箱跑器单点 = `harness.runFlowScript`**（AD-6；Phase 4–8 的 pipeline/loop/retry 断言共用，禁第二份）：把 `flowGenerate` 的产物放进 `new Function` 沙箱真跑，记录 `meta/phases/logs/calls/workflows/order/ret`。断言"图=脚本"一律看**调用序列与参数**，不做文本比对。
...
- **运行/分发命令拼装单点 `flowCommands`**（1.2.52；1.2.59 起多一个 `projectWritten` 参数）：首次 / 恢复（`resumeFromRunId: 'wf_…'` 占位）/ 分发（项目 `.claude/workflows/` 与个人 `"$HOME/.claude/workflows/"` 两条 `cp`；**项目那份已由保存时自动写好时只留个人一条**）三种形态只此一处，UI（`renderRunBox`，切语言重绘）与测试都调它。**个人位置必须写 `"$HOME/…"`——单引号会掐掉 `~` 展开**。边界文案随命令区一起渲染（服务只写 `~/.claude/cc-viewer/`、执行/恢复/停止在终端），断言钉住"首次/恢复命令不含任何写 `.claude/workflows/` 的动作"（分发是用户手动的 `cp`）。
...
- **真机冒烟**：`?flowsmoke=1` 把十六条断言写进 `document.title`(1.2.53 新增 `map-gen`:真机建图 → 出码含 `pipeline(ARGS.paths, (prev, item, i) => …)`;1.2.46 新增 `connPath-anchored`:虚线自由端在**屏幕上**钉住光标——桩算得出流坐标、算不出"屏幕上钉不钉";`drag-add`:真实 HTML5 `DataTransfer` 拖拽建节点;1.2.50 新增 `meta-v2`:`whenToUse` 输入框与可编辑阶段带在真实 DOM 里回填/出码)（无头桩给不出真实布局，连线吸附/拖拽阈值/滚轮缩放只能浏览器实证）。三个已踩的**假失败/假挂死**：① 用自适应缩放的 7 卡起手图会把 handle 推到窗口外（`elementFromPoint` 拿不到）；② 拿图里已存在的节点对测"加边"，会被自己的去重单点静默吃掉；③ `#fConn` 常驻无限 CSS 动画 → 无头虚拟时钟永不空闲 → `--dump-dom` **挂死**（冒烟入口自带禁用动画的 `<style>`，1.2.46）。**`drag-move` 在虚拟时钟下有抖动**(1.2.58 实测:同一构建偶发 ✗,连跑两次即 OK)——它是合成事件的时序敏感项,真机拖拽由 `tests/browser/test_drag_real.ts` 覆盖,冒烟偶发单条 ✗ 先复跑再判定。**它还会给出假绿灯**：冒烟全用合成事件，绕开浏览器的 pointer→mouse 兼容派生——1.2.47「连线不收尾」在它十断言全绿时真机全坏 → 交互类回归进 `tests/browser/test_connect_real.ts`（CDP 真实输入）。
...
#   期望 SMOKE OK ✓nodes ✓contract-C3C4C5 ✓connPath-anchored ✓connect+1 ✓connPath-drawn ✓connPath-cleared ✓drag-move ✓zoom ✓drag-add ✓meta-v2 ✓map-gen ✓branch-gen ✓loop-gen ✓code-gen ✓field-chips ✓embed ✓lib-open ✓gen
#   （--proxy-server 让外网字体请求快速失败，否则 load 事件挂住 --dump-dom；localhost 默认不走代理）
#   ⚠ Chrome 152 起 `--headless=new` 会**挂死到超时**（连 data: 空页都 dump 不出来，2026-09-09 实测）——用 `--headless`。
#   `drag-move` 偶发 ✗ 是虚拟时钟下的已知抖动（1.2.58 记载），复跑一次即 OK；真机拖拽由 drag_real 套件覆盖。
