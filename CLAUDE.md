...
- **运行/分发命令拼装单点 `flowCommands`**（1.2.52，`45-flowgen.ts` 纯函数）：首次 / 恢复（`resumeFromRunId: 'wf_…'` 占位）/ 分发（项目 `.claude/workflows/` 与个人 `"$HOME/.claude/workflows/"` 两条 `cp`）三种形态只此一处，UI（`renderRunBox`，切语言重绘）与测试都调它。**个人位置必须写 `"$HOME/…"`——单引号会掐掉 `~` 展开**。边界文案随命令区一起渲染（服务只写 `~/.claude/cc-viewer/`、执行/恢复/停止在终端），断言钉住"首次/恢复命令不含任何写 `.claude/workflows/` 的动作"（分发是用户手动的 `cp`）。
