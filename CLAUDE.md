...
- Workflow 全功能对齐调研：`docs/workflow-alignment-research.md`（**v2.0，2026-09-09 收口版**，证据基座 **Claude Code 2.1.263**、实施基线 **Lucid 1.2.63**：CC Workflow 能力分四层——L1 脚本语言面（`meta{name,description,whenToUse,phases[{title,detail,model?}]}` 6 字段、`agent(prompt,{label,phase,schema,model,effort,isolation,agentType})` 7 选项 + 3 未文档化、`pipeline`(无栅栏,回调收 `(prev,orig,index)`)、`parallel`(栅栏,null 不 reject)、`phase`/`log`/`args`/**`budget{total,spent(),remaining()}` 硬上限**/**`workflow()` 子流仅一层**、沙箱禁 `Date.now/Math.random/new Date` 与 `import()`）、L2 运行面（journal 的 `v2:<sha256>` 即 resume 缓存键、实测并发上限 16、`meta.phases` 单独就能出阶段进度）、L3 调用面（工具入参 `script|name|scriptPath|args|resumeFromRunId`）、L4 分发面（`.claude/workflows` / `~/.claude/workflows` / 插件 `workflows` 清单字段 + `/plugin:name` 命名空间）；**证据源=官方页 curl + `strings` 还原内置 `workflow-authoring` 技能全文（脚本 API 唯一权威，官方页列不全）+ 本机 5 次真实运行 + 官方插件 7 份脚本**；**对齐设计=三层表达 + 一条诚实出口**（A 结构层可视化 / B 组合层半可视化 `map`=pipeline、`branch`=if、`loop`=while、`retry`/`log`/`subflow` / C 代码逃生舱 `code` 节点 / S 仅呈现：命令区·成本条·文案），**语义覆盖 100% 靠 A∪B∪C；"可视化覆盖率"必须带分母——结构构造 10/10、agent 选项 7/7、meta 6/6 全部可视化，残留（任意表达式/深层嵌套/budget 动态规模/未文档化 opts）刻意归 C 层**（v1.0 的"~85%"无分母，v2.0 已废弃）；实施记录 X6–X12 ↔ 版本 ↔ commit 表、对齐矩阵 35 行、单点清单、866 断言测试矩阵；**覆盖核对清单在 §2.5，逐条"到没到 100%"看它**）
