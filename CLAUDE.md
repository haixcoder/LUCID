...
| HTTP 端点 | `web.class H` | API：`/api/runs`（响应含 `projects[]`＝回看窗口内有会话活动的全部项目，项目下拉数据源，`scan.projects_in_window()` 与 `scan()` 同活动度口径；`tasks`＝`sessions.tasks_summary()` 回看窗口任务精确总数(按输入自身时间戳落窗)+按项目小计,TASKS 仪表数据源） `/api/sessions` `/api/agent` `/api/subagent` `/api/config` `/api/config/save`（含 `notifyInput` 档位，缺省值不改）`/api/config/test`（body `{"kind":"input_required"}` 按等待型发） |
