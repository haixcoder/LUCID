...
- **草稿版本判定两端同源**：后端 `web.draft_ver_ok`（只认整数白名单 `DRAFT_VERS=(1,2)`，`bool` 是 `int` 子类须显式排除）与前端 `loadFlowDrafts` 守卫（`v!==1 && v!==2`）是同一口径的镜像，改一端必须同时改另一端 + `test_draft_api.py` / `test_web_api.py` / `test_flow_editor.ts` 三处用例（1.2.50 v2 = 纯加宽：`whenToUse` + `phases[{title,detail}]`，v1 原样载入，未知版本仍拒绝不猜）。
