...
- **可视化编排 Workflow(1.2.43,入口按项目 1.2.44)**:「✦ 编排」**只在项目下拉选中某个具体项目时出现**(选「◆ 全部项目」时没有这个入口——workflow 的草稿落点、提示词上下文和终端执行目录都要一个确定的项目);点开即是画布 —— 用 Start / Agent / Return 三种积木拖节点、连线成一张 DAG,右侧**实时生成**可执行的 Workflow 脚本(拓扑分层→顺序 `await`,同层多节点→`parallel` 栅栏并各带 `opts.phase`,`{{nX}}` 占位符→模板插值、`{{nX.字段}}` 取结构化输出的某个字段(1.2.60,节点卡上有可点芯片),phase 首现去重→`meta.phases`);「保存草稿」写进 `~/.claude/cc-viewer/drafts/<项目>/<名>.js`(+ 同名 `.json` 图草稿,供回载再编辑),执行仍在目标项目终端跑 `Workflow({scriptPath, args})` —— **服务不代执行**,跑起来的进度由既有扫描自动成卡并嵌回发起会话。顶部 PHASES 条就是这张图将生成的阶段(与运行卡同一套芯片语言),节点卡上的 `n2` 徽章即下游 prompt 要引用的变量名;Del 删选中、点连线删边、⌘/Ctrl+S 保存、关掉页面前的编辑自动存盘并询问恢复
...
**编排器(✦ 编排,1.2.43;1.2.50–1.2.64 对齐全功能)**:全屏模态,**只在你选中了具体项目时存在**(「◆ 全部项目」下入口隐藏,悬停提示会说明原因)。左栏组件板 —— Agent / Map / Branch / Loop / Log / Code / Subflow / Start / Return(点或拖入画布,键盘 Tab+Enter 也可),中间画布(滚轮缩放、拖空白平移、拖卡片移动、右圆点拖到左圆点连线;Branch 有 `true`/`false` 两个出把,Loop 有 `body`/`out` 两个出把,把体末连回 Loop 节点即成环),右侧产物区实时出脚本 —— 顶栏 `PHASES` 条**可编辑**(标题 + 说明 + 该阶段的 `model` 标注,1.2.63;留空则回落 `phase||label` 首现去重;标题与任何 agent 的 `phase` 都不同名时给警告——运行期会多出一个空进度组,1.2.64),读数条给 `L 行数 · 字节 · ◆ 段数`,成本条按「代理数 × 循环上界」估算(分支按较多的一臂计;Map 条目基数/子流内部代理/重试不计,title 里写明,1.2.64)。**Map 节点与 Agent 节点共用同一份选项字段**(它的级 1 本身就是一次 agent 调用,1.2.64);自定义 `claude-*` 模型 id 在下拉里不会被吞掉(1.2.64)。**字段选择(1.2.60)**:上游节点若声明了 schema(结构化输出),它变成下游 prompt 里的一排可点芯片(`n2.title` / `n2.tags`),点一下就把 `{{n2.title}}` 插到光标处——生成 `${n2?.title}`(可选链:上游被跳过时不会炸整张图);没有 schema 的上游给"整节点"芯片,map 的回调模板给 `{{item}}`/`{{index}}`。校验不过就**不出脚本**:缺 Start/Return、非法环(只有 Loop 回边可以成环)、孤立节点、`{{nX}}` 引用非上游或泄漏出 map/branch/loop 区域、`{{nX}}` 指向带 schema 的节点(会得到 `[object Object]`)、占位符写法不合法(如 `{{n2.1bad}}`)、`start`/`index` 取字段、schema 非法 JSON、model/effort 不在白名单、Code 片段含 `import(` 或 `Date.now()/Math.random()/new Date()`,逐条列在红色错误块里(定高可滚动,不裁);另有一类**警告**(未知全局、取字段但上游没声明 schema / 字段名不在 schema 里)显示在单独的黄色条里,**不拦生成**。保存后命令区给出首次运行 / 恢复 / 分发三条命令;同项目下的历史草稿在「载入草稿…」下拉里可回载再编辑;同名草稿内容不同时**默认并存**不覆盖(需要覆盖时才会亮出「覆盖原稿」)。
