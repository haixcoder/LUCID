...
- **可视化编排 Workflow(1.2.43,入口按项目 1.2.44)**:「✦ 编排」**只在项目下拉选中某个具体项目时出现**(选「◆ 全部项目」时没有这个入口——workflow 的草稿落点、提示词上下文和终端执行目录都要一个确定的项目;未选中时按钮原位常显一行提示,告知「先在上方选一个具体项目」);点开即是画布 —— 用 Start / Agent / Return 三种积木拖节点、连线成一张 DAG,右侧**实时生成**可执行的 Workflow 脚本(拓扑分层→顺序 `await`,同层多节点→`parallel` 栅栏并各带 `opts.phase`,`{{nX}}` 占位符→模板插值、`{{nX.字段}}` 取结构化输出的某个字段(1.2.60,节点卡上有可点芯片),phase 首现去重→`meta.phases`);「保存草稿」写进 `~/.claude/cc-viewer/drafts/<项目>/<名>.js`(+ 同名 `.json` 图草稿,供回载再编辑),执行仍在目标项目终端跑 `Workflow({scriptPath, args})` —— **服务不代执行**,跑起来的进度由既有扫描自动成卡并嵌回发起会话。**「工作流库」(1.2.71)**:顶栏把之前构建过的工作流聚到一处 —— 本项目/其他项目的图草稿、选中项目的 `.claude/workflows/*.js`、个人 `~/.claude/workflows/*.js` 与历史运行过的 workflow;有图的(草稿 JSON,或保存时内嵌进执行件尾部的 `// lucid-graph:` 注释)直接无损上画布;只有脚本的按生成器文法**反解 + 逐字节复核**,复核不过就只读展示全文并写明原因(不给假图);载入其他项目的工作流时编辑器上下文随之切换,保存落回原项目。顶部 PHASES 条就是这张图将生成的阶段(与运行卡同一套芯片语言),节点卡上的 `n2` 徽章即下游 prompt 要引用的变量名;Del 删选中、点连线删边、⌘/Ctrl+S 保存、关掉页面前的编辑自动存盘并询问恢复
...
- **隐私友好**：只监听 `127.0.0.1`；读取只发生在你的 Claude 数据目录（转录/运行记录，以及工作流库枚举的那几类脚本），写入只有 `~/.claude/cc-viewer/`（配置 / PID / 去重记录 / 草稿）与保存时那份项目 workflow 副本
...
**顶部**：仪表盘（运行 / 任务 / 运行中 / 已完成 / 异常 五项计数，标签随界面语言切换，「运行中」悬停说明=正在执行的 run、与会话区「活跃」不是一回事，「异常」悬停列出=失败/错误/陈旧/中止/终止/超时 的聚合——统计的是当前筛选视图；TASKS = 主 agent 被调用的任务总次数，对回看窗口按完整转录精确计数、以每条输入自身时间戳落窗（不再有 256KB 尾窗漏计；跨窗口的长会话只计窗口内的任务；卡顶「任务 N」显示该会话的完整精确值）；有项目/搜索过滤时 RUNS 与 TASKS 显示「命中/总数」并悬停说明，避免误读为总数——项目过滤的命中即窗口内精确小计，搜索只能命中视图内会话）、项目筛选（列表 = 回看窗口内有会话活动的全部项目，不只列有 workflow 运行的）、全文搜索、自动刷新开关、版本角标（构建戳=页面代码指纹，悬停有说明；插件版本见设置 07）；右上「⚙ 设置」：01 通知钩子（含「等待输入通知」档位与 ⏸ 分型测试按钮）/ 02 端口 / 03 主题 / 04 语言 / 05 自动扫描 / 06 回看窗口 / 07 插件版本（本服务正在运行的副本版本——升级插件后需重启服务才更新；读不到清单时显示「未知」），其中主题·语言·自动扫描即点即生效，其余由底栏统一保存。顶栏常显「更新于 HH:MM:SS」；轮询失败出现链路中断横幅、数据区降透明度，恢复后自动还原。窄屏(≤640px)为紧凑布局：顶栏换行、宽表格横向滚动。
...
**编排器(✦ 编排,1.2.43;1.2.50–1.2.64 对齐全功能)**:全屏模态,**只在你选中了具体项目时存在**(「◆ 全部项目」下入口隐藏,原位常显一行提示说明怎么出现)。左栏组件板 —— Agent / Map / Branch / Loop / Log / Code / Subflow / Start / Return(点或拖入画布,键盘 Tab+Enter 也可),中间画布(滚轮缩放、拖空白平移、拖卡片移动、右圆点拖到左圆点连线;Branch 有 `true`/`false` 两个出把,Loop 有 `body`/`out` 两个出把,把体末连回 Loop 节点即成环),右侧产物区实时出脚本 —— 顶栏 `PHASES` 条**可编辑**(标题 + 说明 + 该阶段的 `model` 标注,1.2.63;留空则回落 `phase||label` 首现去重;标题与任何 agent 的 `phase` 都不同名时给警告——运行期会多出一个空进度组,1.2.64),读数条给 `L 行数 · 字节 · ◆ 段数`,成本条按「代理数 × 循环上界」估算(分支按较多的一臂计;Map 条目基数/子流内部代理/重试不计,title 里写明,1.2.64)。**Map 节点与 Agent 节点共用同一份选项字段**(它的级 1 本身就是一次 agent 调用,1.2.64);自定义 `claude-*` 模型 id 在下拉里不会被吞掉(1.2.64)。**字段选择(1.2.60)**:上游节点若声明了 schema(结构化输出),它变成下游 prompt 里的一排可点芯片(`n2.title` / `n2.tags`),点一下就把 `{{n2.title}}` 插到光标处——生成 `${n2?.title}`(可选链:上游被跳过时不会炸整张图);没有 schema 的上游给"整节点"芯片,map 的回调模板给 `{{item}}`/`{{index}}`。校验不过就**不出脚本**:缺 Start/Return、非法环(只有 Loop 回边可以成环)、孤立节点、`{{nX}}` 引用非上游或泄漏出 map/branch/loop 区域、`{{nX}}` 指向带 schema 的节点(会得到 `[object Object]`)、占位符写法不合法(如 `{{n2.1bad}}`)、`start`/`index` 取字段、schema 非法 JSON、model/effort 不在白名单、Code 片段含 `import(` 或 `Date.now()/Math.random()/new Date()`,逐条列在红色错误块里(定高可滚动,不裁);另有一类**警告**(未知全局、取字段但上游没声明 schema / 字段名不在 schema 里)显示在单独的黄色条里,**不拦生成**。保存后命令区给出首次运行 / 恢复 / 分发三条命令;**工作流库(1.2.71)**把「之前已经构建的工作流」聚到顶栏一处:本项目/其他项目草稿、选中项目的 `.claude/workflows` 脚本、个人 `~/.claude/workflows`、历史运行(按名去重保留最近一次)——有图(草稿 JSON 或执行件里的内嵌图)直接无损上画布;只有脚本的先**反解 + 逐字节复核**(通过才上画布,布局重排;args schema 只还原 required 名单),反解不了就只读展示全文并写明原因;载入其他项目的草稿会**切换编辑器上下文**(头部/草稿库/保存落点/命令区全跟随),保存落回原项目;同名草稿内容不同时**默认并存**不覆盖(需要覆盖时才会亮出「覆盖原稿」)。
...
| `GET /api/config` | 当前 webhook 配置 + 最近推送结果 + `plugin:{name,version}`（**本服务运行副本自己**的 `.claude-plugin/plugin.json`，设置中心 07 模块与顶栏角标 tooltip 的数据源；读不到为空串） |
...
| `GET /api/workflows?proj=<cwd>` | 工作流库列表(编排器「工作流库…」的数据源):`{items:[{src,name,desc,mtime,graph,path,js,…}]}`;`src`∈{`draft` 跨项目草稿, `proj` 选中项目的 `.claude/workflows` 脚本, `home` 个人 `~/.claude/workflows` 脚本, `run` 历史运行记录(按 `workflowName` 去重、保留最近一次)};`graph` = 能否直接上画布(草稿恒真;脚本看尾部内嵌图注释)。**只读枚举**,总上限 300、单目录 100、运行记录扫描 400 |
| `GET /api/workflow?path=<abs>` | 工作流库单件读取(**白名单根内才读**,与列表同一批根):草稿 `.json` → `{kind:'draft', graph}`;运行记录 `wf_*.json` → `{kind:'run', script}`(取记录内 `script` 字段);脚本 `.js` → `{kind:'script', script, embed, truncated}`(读取上限 1MB,截断如实上报)。白名单:`cc-viewer/drafts/**`、`~/.claude/projects/**/workflows/{wf_*.json,scripts/*.js}`、任意 `<dir>/.claude/workflows/*.js`;白名单外一律 404,不泄露内容 |
