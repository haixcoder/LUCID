...
**编排器(✦ 编排,1.2.43;1.2.50–1.2.58 对齐全功能)**:全屏模态,**只在你选中了具体项目时存在**(「◆ 全部项目」下入口隐藏,悬停提示会说明原因)。左栏组件板 —— Agent / Map / Branch / Loop / Log / Code / Subflow / Start / Return(点或拖入画布,键盘 Tab+Enter 也可),中间画布(滚轮缩放、拖空白平移、拖卡片移动、右圆点拖到左圆点连线;Branch 有 `true`/`false` 两个出把,Loop 有 `body`/`out` 两个出把,把体末连回 Loop 节点即成环),右侧产物区实时出脚本 —— 顶栏 `PHASES` 条**可编辑**(标题 + 说明;留空则回落 `phase||label` 首现去重),读数条给 `L 行数 · 字节 · ◆ 段数`,成本条按「代理数 × 循环上界」估算。校验不过就**不出脚本**:缺 Start/Return、非法环(只有 Loop 回边可以成环)、孤立节点、`{{nX}}` 引用非上游或泄漏出 map/branch/loop 区域、schema 非法 JSON、model/effort 不在白名单、Code 片段含 `import(` 或 `Date.now()/Math.random()/new Date()`,逐条列在红色错误块里(定高可滚动,不裁);Code 片段另有一类**警告**(未知全局——幻觉 API 唯一可能溜进来的地方)显示在单独的黄色条里,**不拦生成**。保存后命令区给出首次运行 / 恢复 / 分发三条命令;同项目下的历史草稿在「载入草稿…」下拉里可回载再编辑;同名草稿内容不同时**默认并存**不覆盖(需要覆盖时才会亮出「覆盖原稿」)。
