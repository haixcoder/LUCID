...
**顶部**：仪表盘（RUNS / TASKS / LIVE / DONE / ALERT 计数——统计的是当前筛选视图；TASKS = 主 agent 被调用的任务总次数，即视图内各会话「尾窗任务」之和；有项目/搜索过滤时 RUNS 与 TASKS 显示「命中/总数」并悬停说明，避免误读为总数）、项目筛选、全文搜索、自动刷新开关、版本角标；右上「⚙ 设置」：01 通知钩子（含「等待输入通知」档位与 ⏸ 分型测试按钮）/ 02 端口 / 03 主题 / 04 语言 / 05 自动扫描 / 06 回看窗口，其中主题·语言·自动扫描即点即生效，其余由底栏统一保存。
