...
const { load, payload, makeCk, visibleText, RUN_DONE, RUN_LIVE, SESSION_FIX } = require('./harness');
...
function dictKeys(lang) {
  const m = new RegExp('\\n  ' + lang + ': \\{', 'm').exec(i18nSrc);
  if (!m) return null;
  let i = m.index + m[0].length, depth = 1, keys = [];
...
const tpl = fs.readFileSync(path.join(__dirname, '..', '..', 'frontend', 'template.html'), 'utf8');
