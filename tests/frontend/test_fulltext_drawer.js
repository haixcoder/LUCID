...
process.env.TZ = 'UTC';
const { load, payload, makeCk, opts0ver, RUN_DONE, SESSION_FIX } = require('./harness');
...
(async function main() {
  ...
  ck('重建后渲染即感知 FULL(标签直接「全文」)', d1b && /全文/.test((d1b.querySelectorAll('.pane')[0] || { dataset: {} }).dataset.t || ''),
