...
process.env.TZ = 'UTC';
const { load, makeCk, payload, SESSION_FIX } = require('./harness');
...
(async function main() {
  ...
  const stepOf = mid => sessEl.querySelector(`details[data-k="${SID}:${mid}"]`);
  const headOf = u => sessEl.querySelector(`details[data-k="${SID}:prompt:${u}"]`);
