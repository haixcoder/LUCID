...
import { load, makeCk, payload, type Env, RUN_DONE, RUN_LIVE, SESSION_FIX, El } from './harness.ts';
const { ck, done } = makeCk();

const GAP = 10, SESS_BASE = 200, SESS_PER_DET = 30, RUN_H = 300;
let WIN: Env['win'] | null = null;
function cardH(c: El): number {
...
function docTopOf(el: El): number {  // 文档流:#sess 区整体在上,#list 在下;区内按前序兄弟累加
...
(async function main() {
  ...
  ElProto.getBoundingClientRect = function (this: El) {
