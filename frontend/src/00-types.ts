...
interface Step { msgId: string; turn?: string; tools: string[]; text: string; toolHint?: string; model: string | null; tokIn: number; tokOut: number; ts: string | null }  // turn=开启所在回合的真人输入记录 uuid(''=尾窗之前的输入;后端 _user_prompt 单点判定,前端不猜边界);toolHint=后端给的"工具调用摘要"(如 'Bash: git log',≤80 字;取不到为 '',旧后端无此字段)
interface Subagent {
