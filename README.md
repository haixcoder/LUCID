...
- **Visual Workflow composition (1.2.43)**: the top-bar **✦ Compose** opens a canvas where Start / Agent / Return blocks are dragged and wired into a DAG, and the script is **generated live** on the right (topological layers → sequential `await`; several nodes in one layer → a `parallel` barrier where each agent carries its own `opts.phase`; `{{nX}}` placeholders → template interpolation; first-seen phase names → `meta.phases`). **Save draft** writes `~/.claude/cc-viewer/drafts/<project>/<name>.js` plus a same-named `.json` graph draft for reload-and-edit; execution still happens in your terminal via `Workflow({scriptPath, args})` — **the server never executes anything for you**, and the run then shows up as a card through the existing scanner, embedded back into its originating session. The `PHASES` chip strip up top is exactly what this graph will generate (same chip language as run cards), the `n2` badge on a card is the variable downstream prompts reference; Del removes the selection, clicking an edge deletes it, ⌘/Ctrl+S saves, and unsaved edits autosave per project and ask before restoring
