...
|----------|-------------|
| `GET /api/runs` | full run snapshot (`{now, runs[], projects[]}`; in-progress runs rebuilt live from journal/transcripts; `projects` = every project with session activity inside the lookback window — including projects that have no workflow runs and aren't currently live) |
