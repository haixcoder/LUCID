...
- **Visual Workflow composition (1.2.43; entry gated per project in 1.2.44)**: the top-bar **✦ Compose** appears **only when a specific project is selected** (under *◆ All projects* there is no such entry — a workflow draft folder, prompt context and terminal working directory all need one concrete project). It opens a canvas where Start / Agent / Return blocks are dragged and wired into a DAG, and the script is **generated live** on the right (topological layers → sequential `await`; several nodes in one layer → a `parallel` barrier where each agent carries its own `opts.phase`; `{{nX}}` placeholders → template interpolation; first-seen phase names → `meta.phases`). **Save draft** writes `~/.claude/cc-viewer/drafts/<project>/<name>.js` plus a same-named `.json` graph draft for reload-and-edit; execution still happens in your terminal via `Workflow({scriptPath, args})` — **the server never executes anything for you**, and the run then shows up as a card through the existing scanner, embedded back into its originating session. The `PHASES` chip strip up top is exactly what this graph will generate (same chip language as run cards), the `n2` badge on a card is the variable downstream prompts reference; Del removes the selection, clicking an edge deletes it, ⌘/Ctrl+S saves, and unsaved edits autosave per project and ask before restoring
...
**Composer (✦ Compose, 1.2.43)**: a full-screen modal that **exists only for a selected project** (hidden under *◆ All projects*, with a tooltip explaining why). Left: three block types (click or drag onto the canvas; Tab + Enter works too). Middle: the canvas (wheel to zoom, drag empty space to pan, drag a card to move it, drag from a card's right dot to another's left dot to connect). Right: the artifact — the header `PHASES` chips list the stages this graph will generate and the readout shows `L lines · bytes · ◆ stages`. An invalid graph deliberately **produces no script**: missing Start/Return, a cycle, an unconnected node, a `{{nX}}` that isn't upstream, malformed schema JSON or an off-whitelist model are each listed in the red box (fixed height, scrollable, never truncated). After saving, copy the `Workflow({ scriptPath: '<absolute path>', args: '<input>' })` command straight into a terminal; earlier drafts of the same project sit in the *Load draft…* dropdown; when a saved draft's content differs, the server **keeps both** as `<name>-<sha8>.*` and only then reveals "Overwrite original".
...
- the composer emits **acyclic graphs with stage barriers** only: several in-edges on one node means "wait for all of them" (a join), and loops / conditional branches / pipeline-style containers are not supported yet (the validator errors instead of guessing). The server never executes workflows — the entry point stays `Workflow({scriptPath})` in your terminal (the control plane is out of this viewer's scope). The draft list is exactly "what's in the directory"; rename/delete UI is deferred to a later batch.
