...
| `POST /api/draft/save` | save a draft: `{name, draft (graph JSON with v==1), script, overwrite?}` → the executable `<name>.js` and graph `<name>.json` are written as a pair, atomically, under `~/.claude/cc-viewer/drafts/<sanitize>-<hash6>/`; `name` is whitelist-validated, `cwd` only feeds the directory's basename+hash (never a path component), script capped at 1MB, differing content under the same name **coexists** as `<name>-<sha8>.*`; returns `{ok, msg, path (absolute), sha}` |
