...
- **Workflows inline in the session timeline**: a workflow launched by the main agent is no longer stranded in the run section far below — its full run card is embedded back into the launching session card's turn timeline **at step granularity: it sits immediately after the step that launched it**, not lumped at the end of the turn (a turn with a dozen steps used to push every card it started past all of them). Inside a turn, run cards and step rows are sorted together by time; a run started before the first turn keeps its place ahead of the groups. An embedded card is removed from the run section (never shown twice); if the launching session has no card in view (out of window / filtered out), the run stays a standalone card there. The RUNS / LIVE / DONE / ALERT gauges always count the full set (embedded + standalone) — however many run cards the page shows, that's what the gauge says
- **Visual Workflow composition (1.2.43; full Workflow feature alignment completed across 1.2.50–1.2.64)**: the top-bar **✦ Compose** appears **only when a specific project is selected** (under *◆ All projects* there is no such entry — a workflow draft folder, prompt context and terminal working directory all need one concrete project). Drag blocks onto the canvas and wire them into a graph; the script is **generated live** on the right. **Block types**: Start / Agent / **Map** (`pipeline`, one callback per downstream level, `{{item}}`/`{{index}}`; a fan-out level converging on one Merge becomes `() => parallel([…])`) / **Branch** (`if/else`, true+false must end at the same Merge or the same Return) / **Loop** (`while (cond && round < maxRounds)`, optional `budget` guard, wire the body end back to the Loop node) / **Merge** (region join, runs nothing itself) / **Log** (`log()` narration) / **Code** (an escape hatch: any JS in the sandbox, marked with a red “not part of the visual semantics” banner) / **Subflow** (`workflow(name | {scriptPath}, args)`, one level only). **Agent options**: model / effort (5 levels) / agentType (candidates enumerated from `~/.claude/agents` + installed plugins) / isolation:worktree / schema / retry(n, backoff ms → a single `$retry` helper with exponential backoff) — the **Map node gets the same fields** (its level 1 *is* an agent call), and a custom `claude-*` model id survives the dropdown (1.2.64). The **args panel** declares a JSON Schema + example + required flag and generates `ARGS` parsing plus a preflight check; the **cost bar** shows `◆agents · concurrency ≤16 · rounds ≤N · budget guard` and turns red at 25+ (branch arms counted by the larger arm; Map item cardinality, subflow agents and retries are disclosed as excluded, 1.2.64). **Save draft** writes `~/.claude/cc-viewer/drafts/<project>/<name>.js` plus a same-named `.json` graph draft, **and also drops the executable into your selected project's `.claude/workflows/<name>.js`** (same name = overwritten; the draft copy still keeps both versions). The command area then offers **first run / resume (`resumeFromRunId`) / distribute** — the project line disappears once the server has written it, leaving only the personal-location `cp`. Execution still happens in your terminal, **the server never executes anything for you**. The **workflow library (1.2.71)** in the top bar gathers everything you have built before — graph drafts from this and other projects, the selected project's `.claude/workflows/*.js`, personal `~/.claude/workflows/*.js`, and past runs (deduped by name, latest kept). Items that carry a graph (the draft JSON, or a `// lucid-graph:` comment embedded in the executable on save) open losslessly onto the canvas; script-only items are first **reverse-parsed and byte-verified**, and when that fails the full script is shown read-only with the reason (never a fake graph). Loading another project's workflow switches the editor context, and saving writes back into that project. The `PHASES` strip is editable (title + detail + a per-phase `model` annotation, 1.2.63; a title that no agent's `phase` matches gets a warning, since the run would show an extra empty group, 1.2.64) and is exactly what the script's `meta.phases` will say; the `n2` badge on a card is the variable downstream prompts reference, and a node with a schema turns its fields into clickable `{{n2.field}}` chips (1.2.60)
...
- **Privacy-friendly**: binds `127.0.0.1` only; reading happens only inside your Claude data directories (transcripts, run records, and the workflow scripts the library enumerates), writing only `~/.claude/cc-viewer/` (config / PID / dedupe record / drafts) plus the project workflow copy made on save
...
**Top bar**: dashboard (RUNS / TASKS / RUNNING / DONE / ALERT counters — labels follow the UI language, and RUNNING / ALERT carry hover scopes (RUNNING = workflow runs currently executing, distinct from the session-level "live" below; ALERT = aggregate of failed / error / stale / aborted / killed / timeout); they count the current filtered view; TASKS = total times the main agent was invoked with a task, counted exactly over every session in the lookback window by each prompt's own timestamp (no 256KB tail-window truncation; a session that spans the boundary only contributes its in-window tasks; each card's "tasks N" shows that session's full exact count); with a project/search filter active, RUNS and TASKS show `hits/total` with a hover explanation, so filtered-away runs are never mistaken for bad data — with a project filter the hits are the window-wide exact count, while search can only match sessions currently in view), project filter (the list covers every project with session activity in the lookback window — not just ones with workflow runs), full-text search, auto-refresh toggle, version badge (a build stamp — page-code fingerprint, hover for details; the plugin version lives in Settings 07); the bar also carries an "updated at HH:MM:SS" stamp — if polling fails, a link-down banner appears and the data area dims until the next successful poll; on narrow screens (≤640px) the bar wraps and wide tables scroll sideways. Top-right "⚙ Settings": 01 webhook (incl. the "input-required notification" tier and ⏸ per-kind test buttons) / 02 port / 03 theme / 04 language / 05 auto-scan / 06 lookback window / 07 plugin version (of the copy this service is running — restart the service to pick up an upgrade; shows "unknown" when the manifest can't be read) — theme, language and auto-scan take effect at once, everything else is saved by the footer bar.
...
**Composer (✦ Compose, 1.2.43; aligned with the full Workflow surface in 1.2.50–1.2.64)**: a full-screen modal that **exists only for a selected project** (hidden under *◆ All projects* — an always-visible hint next to its spot tells you to pick a specific project first). Left: the block palette — Agent / Map / Branch / Loop / Log / Code / Subflow / Start / Return (click or drag onto the canvas; Tab + Enter works too). Middle: the canvas (wheel to zoom, drag empty space to pan, drag a card to move it, drag from a card's right dot to another's left dot to connect; Branch has `true`/`false` dots, Loop has `body`/`out` dots and you close the cycle by wiring the body end back into the Loop node). Right: the artifact — the `PHASES` strip is **editable** (title + detail + a per-phase `model` annotation, 1.2.63; leave it empty and it falls back to first-seen `phase||label`), the readout shows `L lines · bytes · ◆ stages`, and the cost bar estimates agents × loop bounds. **Field picking (1.2.60)**: when an upstream node declares a schema (structured output), it turns into a row of clickable chips on the downstream prompt (`n2.title`, `n2.tags`); clicking one inserts `{{n2.title}}` at the cursor, which generates `${n2?.title}` (optional chaining, so a skipped upstream returns null instead of crashing the run). Upstreams without a schema get a whole-node chip, and a map's callback template gets `{{item}}`/`{{index}}`. An invalid graph deliberately **produces no script**: missing Start/Return, an illegal cycle (only Loop back-edges may close a cycle), an unconnected node, a `{{nX}}` that isn't upstream or that leaks out of a map/branch/loop region, `{{nX}}` pointing at a schema node (that would yield `[object Object]`), a malformed placeholder such as `{{n2.1bad}}`, field access on `start`/`index`, malformed schema JSON, an off-whitelist model/effort, `import(` or `Date.now()/Math.random()/new Date()` in a Code block are each listed in the red box (fixed height, scrollable, never truncated); a second, **non-blocking** yellow strip carries warnings (unknown globals; a field taken from an upstream with no schema, or a field name missing from its `properties`). After saving, the command area gives first-run, resume and distribution commands. The **workflow library (1.2.71)** in the top bar gathers everything built before: drafts of this and other projects, the selected project's `.claude/workflows` scripts, personal `~/.claude/workflows` scripts and past runs (deduped by name, latest kept). Items with a graph (draft JSON, or the graph embedded in the executable on save) open losslessly onto the canvas; script-only items are first **reverse-parsed and byte-verified** (layout re-arranged; the args schema is rebuilt from its required list only), and if that fails the full script is shown read-only with the reason. Loading another project's draft **switches the editor context** (header, draft store, save target and command area all follow) and saves back into that project. When a saved draft's content differs, the server **keeps both** as `<name>-<sha8>.*` and only then reveals "Overwrite original".
...
| `GET /api/config` | current webhook config + recent push results + `plugin:{name,version}` (the manifest of **the copy this service is running** — data source of Settings module 07 and the top-bar badge tooltip; empty strings when unreadable) |
...
| `GET /api/agents` | agentType candidates for the composer's dropdown: `{types:[…]}` — `~/.claude/agents/*.md` plus installed plugins as `<plugin>:<name>`, deduped, cap 200. **Read-only enumeration** (no new write path) |
| `GET /api/drafts?proj=<cwd>` | draft list for that project (the composer's *Load draft* source): `{drafts:[{name, meta, mtime, js, draft}]}` where `draft` is the full graph JSON. **The directory listing is the verdict** (no second definition), unreadable files are skipped rather than failing the list, cap 200 |
| `GET /api/workflows?proj=<cwd>` | workflow-library listing (the *Workflow library…* source): `{items:[{src,name,desc,mtime,graph,path,js,…}]}` with `src` ∈ {`draft` cross-project graph drafts, `proj` scripts in the selected project's `.claude/workflows`, `home` personal `~/.claude/workflows` scripts, `run` past runs deduped by `workflowName` keeping the latest}; `graph` = can it open directly on the canvas (always true for drafts; for scripts it means the trailing embedded-graph comment is present). **Read-only enumeration**, overall cap 300, 100 per directory, 400 run records scanned |
| `GET /api/workflow?path=<abs>` | read one library item (**only inside the allow-listed roots**, same set as the listing): draft `.json` → `{kind:'draft', graph}`; run record `wf_*.json` → `{kind:'run', script}` (its inline `script` field); script `.js` → `{kind:'script', script, embed, truncated}` (1 MB read cap, truncation reported honestly). Allow-list: `cc-viewer/drafts/**`, `~/.claude/projects/**/workflows/{wf_*.json,scripts/*.js}`, any `<dir>/.claude/workflows/*.js`; everything else is a 404 that leaks nothing |
| `POST /api/draft/save` | save a draft: `{name, draft (graph JSON with v∈{1,2}), script, overwrite?}` → the executable `<name>.js` and graph `<name>.json` are written as a pair, atomically, under `~/.claude/cc-viewer/drafts/<sanitize>-<hash6>/`; `name` is whitelist-validated, `cwd` only feeds the draft directory's basename+hash, script capped at 1MB, differing content under the same name **coexists** as `<name>-<sha8>.*`. **Since 1.2.59 it also writes the executable to `<cwd>/.claude/workflows/<name>.js`** (same name overwritten; three-way symlink refusal; a missing cwd is skipped with `wfErr`); returns `{ok, msg, path (absolute), sha, wfPath, wfErr}` |
| `GET /static/xyflow.system.umd.js` | whitelisted static asset (the vendored `@xyflow/system` UMD behind the canvas: 100KB, d3 inlined, committed build artifact — runtime stays dependency-free); **only enumerated file names** resolve, traversal and unlisted names are 404 |
...
- drafts are written only inside `~/.claude/cc-viewer/drafts/` (name whitelist + last-path-segment + realpath prefix — three sanitisation steps); the single write into your project tree is the workflow copy made on save (`<project>/.claude/workflows/<name>.js`); `/static/` exposes only the enumerated vendored file and `/api/workflow` reads only the allow-listed library roots;
...
│   ├── src/*.ts           # 00-types/05-i18n/10-util/20-render/30-app/39-flow-shim/40-flow/45-flowgen, concatenated by filename order into a global script
...
└── scripts/
    ...
    └── ccviewer/          # kernel package (stdlib only)
        ...
        ├── web.py         # HTTP handler (page + JSON API + composer routes)
        ├── static/index.html  # frontend runtime file (built from frontend/; plugin distribution/runtime still need no build step)
        └── static/xyflow.system.umd.js  # vendored @xyflow/system UMD for the canvas (committed build artifact, zero runtime deps)
```
...
| HTTP endpoints | `web.class H` | the page + the JSON APIs above (incl. the composer's /api/drafts, /api/draft/save and the /static/ allowlist) |
...
- the composer supports acyclic graphs (stage barriers: several in-edges on one node means "wait for all of them"), `map`/`pipeline`, `branch`/`merge`, `loop` (the only place a cycle may be closed, via a back-edge into the Loop node) and `code`/`subflow` escape hatches — but expressions (`cond`, `items`, `argsExpr`) are plain text boxes, not a second JS IDE: the graph expresses **structure**, not syntax. The server never executes workflows — the entry point stays `Workflow({scriptPath})` in your terminal (the control plane is out of this viewer's scope). **The one write outside `~/.claude/cc-viewer/`** is the project workflow copy made on save: `<selected project>/.claude/workflows/<name>.js`, refused outright if `.claude`, `.claude/workflows` or the target file is a symlink. The draft list is exactly "what's in the directory"; rename/delete UI is deferred to a later batch. The **workflow library (1.2.71) is read-only**: both listing and single-item reads go through the same allow-listed roots (drafts / `projects/**/workflows` / any `.claude/workflows`), out-of-scope paths return 404, and the write domain is unchanged; on save one graph comment (`// lucid-graph:`) is embedded at the end of the executable so distributed copies can be reloaded losslessly (comments are inert to the Workflow sandbox).
