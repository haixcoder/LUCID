...
- drafts are written only inside `~/.claude/cc-viewer/drafts/` (name whitelist + last-path-segment + realpath prefix — three sanitisation steps) and **never** into `~/.claude/projects/` or your project directory; `/static/` exposes only the enumerated vendored file;
