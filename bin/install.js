...
// ⚠ bin/install.js 是本文件的编译产物(tsc --module commonjs,兼容 node ≥16.7):
//    勿手改 .js——改这里,再 `npm run build`(或 npm install / npm publish 的 prepare 自动重建)。
const node_fs_1 = __importDefault(require("node:fs"));
const node_os_1 = __importDefault(require("node:os"));
const node_path_1 = __importDefault(require("node:path"));
const node_child_process_1 = require("node:child_process");
const PKG_DIR = node_path_1.default.resolve(__dirname, '..'); // bin/ 固定在包根(npm/_npx 缓存布局均成立)
const CONFIG_DIR = process.env.CLAUDE_CONFIG_DIR || node_path_1.default.join(node_os_1.default.homedir(), '.claude');
const STABLE = node_path_1.default.join(CONFIG_DIR, 'plugins', 'marketplaces', 'kw-lucid-npm');
...
function run(args) {
    const p = (0, node_child_process_1.spawnSync)('claude', args, { encoding: 'utf8' });
    if (p.error)
        return { code: -1, out: String(p.error.message || p.error) };
...
for (const f of [node_path_1.default.join('.claude-plugin', 'marketplace.json'), node_path_1.default.join('.claude-plugin', 'plugin.json')]) {
    if (!node_fs_1.default.existsSync(node_path_1.default.join(PKG_DIR, f))) {
        fail('npm 包缺少 ' + f + '(包体损坏?),请 npm cache clean -f 后重试');
    }
}
// 3) 同步到稳定市场目录(先清后拷;STABLE 路径含固定段名 kw-lucid-npm,rm 安全)
node_fs_1.default.mkdirSync(node_path_1.default.dirname(STABLE), { recursive: true });
node_fs_1.default.rmSync(STABLE, { recursive: true, force: true });
node_fs_1.default.mkdirSync(STABLE, { recursive: true });
for (const c of COMPONENTS) {
    const src = node_path_1.default.join(PKG_DIR, c);
    if (node_fs_1.default.existsSync(src))
        node_fs_1.default.cpSync(src, node_path_1.default.join(STABLE, c), { recursive: true });
}
...
if (op.code !== 0)
    op = run(['plugin', 'install', PLUGIN_ID, '-y']);
if (op.code !== 0)
    fail('插件安装失败:\n' + op.out);
